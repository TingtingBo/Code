...
seed_prefix = {'r*' 'r*'};
seed_folder = {'label'};
denoised_images = {'swraMacDe_RestingState1_denoised'};
names = {'Dbach_PNAS_restE1'}; %output
cd(data_path);
...
for i = 1:length(subjects);
    ...
    cfg.data_path = fullfile(data_path, subjects{i}, 'MacDe_RestingState1');
    cfg.save_path = fullfile(save_path,subjects{i});
    cfg.subject_vol = denoised_images{1};
    cfg.name = names{1};
    util_roi_fc_autowb_E1(cfg);
