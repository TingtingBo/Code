...
%     error_rCon = (error_rCon + error_rCon2)/2;
    ...
    hold on;
    set(gcf, 'Visible', 'off')
    p1= plot(plot_series_r, '--rs');
    %p2= plot(plot_series_l, '-.bo');
    errorb(plot_series_r,error_r);
    %errorb(plot_series_l,error_l);
    hold off 
    axis square
    set(p1, 'LineWidth', 4)
    %set(p2, 'LineWidth', 4)
    ...
    ylabel('Z-score');
    M = ['Right V1' 'Left V1'];
%     legend(gca, 'Right V1', 'Left V1');
    
    
        
        
    fig(h,'border', 'off', 'units', 'inches', 'width', 7, 'height', 7 );
    box on
    set(gcf, 'Visible','off')
    cd '/media/store3/Projects/Wess/subjects/Vines_conn'
