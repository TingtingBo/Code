...
%% steps to run this
% change out the cor matrices example profileGrecious_DMN_task.mat or
% profileDbach_PNAS_task.mat
% first select all the way to above and evaluate 
% then evaluate down to con = 1;
% then evaluate to if con == 1 loop; this will clear it all out except for
% con data
% next start at the top and run to subjects = MD
% then evaluate serier to n_subjects
% then evaluate from list down ( dont do con == 1 though). 
% next evaluate the figure maker 
...
for i = 1:n_rois
    ...
    h = figure;
    error_r = std_error_ws(series_r.all_val{i}); % use std error scipt from joe
    error_r2 = std_error_ws(series_l.all_val{i});
    error_r = (error_r + error_r2)/2;
    error_rCon = std_error_ws(storeconr.all_val{i}); 
    error_rCon2 = std_error_ws(storeconl.all_val{i});
    ...
    box on
    set(gcf, 'Visible','off')
    cd '/media/store3/Projects/Wess/subjects/MD_FC_ROI/DMN_Task'
    out_file = strcat(list{i}, num2str(i));
    export_fig(h, eval('out_file'), '-jpeg');
end 
close all    
