...
listin = load('/media/store3/Projects/Wess/subjects/MD_FC_ROI/list_Grecious_DMN_task.mat');
...
for i = 1:n_subjects
    ...
    cd(sub_con_dir);
    if rest == 1
        sub_con = load('profileGrecious_DMN_rest.mat');
    else
        sub_con = load('profileGrecious_DMN_task.mat');
    end
    
    cor = sub_con.correlation.z_score;
    list = load('/media/store3/Projects/Wess/subjects/MD_FC_ROI/list_Grecious_DMN_task.mat');
...
%     error_rCon = (error_rCon + error_rCon2)/2;
   ...
   legend(gca, 'Con', 'MacD');
    ...
    box on
    set(gcf, 'Visible','off')
    cd '/media/store3/Projects/Wess/subjects/MD_FC_ROI/MD_MODULE_DMN'
