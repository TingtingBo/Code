...
% Joseph Grififs 2014 Edited by WB 2014
...
data_dir = '/media/store3/Projects/Wess/subjects/Whole_Brain_MD/L_R_average_md';
n_rois = 1;
conditions = {'mean_roi2_rest_block'};
names = {'mean_roi2_rest_block'};


        cd(data_dir)
        p = spm_select('FPList',data_dir, [ names{1} '.img']);
        out_dir = fullfile( 'SPM_Level2_', [ conditions{1} '_' names{1}]);       
        if isdir(out_dir) ~= 1
            mkdir(out_dir);
        end
